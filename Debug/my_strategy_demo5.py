...
if __name__ == "__main__":
    ...
    """
    code = "EURUSD"
    begin_time = "2011-01-01 00:00:00"
    ...
    dataset = xgb.DMatrix("feature.libsvm?format=libsvm")  # load sample
    # 获取 DMatrix 中的数据和标签
    ...
    model.compile(optimizer=keras.optimizers.Adam(0.001), loss=keras.losses.BinaryFocalCrossentropy(),
                  metrics=["accuracy"])
    model.fit(X_train, y_train, batch_size=1024, epochs=50, verbose=2, validation_data=(X_test, y_test), shuffle=True)
    # 预测测试集
    ...
    print(f"Positive Class Accuracy: {accuracy_positive:.2f}")
